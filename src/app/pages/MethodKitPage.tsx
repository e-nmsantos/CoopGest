...
// ────────────────────────────────────────────────────────────
// Card data — based on MethodKit for Projects (56 cards)
// ────────────────────────────────────────────────────────────
interface MKCard {
  id: string;
  title: string;
              ...
              {filtered.map((card) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-3">
                      <span className={`inline-block text-[10px] font-medium px-2 py-0.5 rounded-full border ${cat?.color || ""}`}>
                        {cat?.label}
                      </span>
                    </div>
                  </Card>
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                <span className="text-4xl">{detailCard.icon}</span>
                <div>
                  <h2 className="text-xl font-bold text-gray-900">{detailCard.titlePt}</h2>
                  <p className="text-sm text-gray-400">{detailCard.title}</p>
                </div>
              </div>
              <button onClick={() => setDetailCard(null)} className="text-gray-400 hover:text-gray-600">
                <X className="size-5" />
              </button>
            </div>

            <div className="mb-4">
              <span className={`inline-block text-xs font-medium px-2.5 py-1 rounded-full border ${CATEGORY_MAP[detailCard.category]?.color || ""}`}>
                {CATEGORY_MAP[detailCard.category]?.label}
              </span>
            </div>

            <p className="text-gray-700 text-sm leading-relaxed mb-6">{detailCard.description}</p>

            <div className="bg-gray-50 rounded-lg p-3 mb-6">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Perguntas guia</p>
              <ul className="text-sm text-gray-700 space-y-1">
                {getGuideQuestions(detailCard.id).map((q, i) => (
            ...
            <div className="flex gap-2">
              <Button
                className="flex-1"
                variant={selectedCards.has(detailCard.id) ? "default" : "outline"}
                onClick={() => toggleCard(detailCard.id)}
              >
                <CheckSquare className="size-4 mr-2" />
                {selectedCards.has(detailCard.id) ? "Remover da seleção" : "Selecionar cartão"}
              </Button>
...
// ────────────────────────────────────────────────────────────
// Guide questions per card
// ────────────────────────────────────────────────────────────
function getGuideQuestions(id: string): string[] {
